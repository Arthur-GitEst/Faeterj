...
function salvarNoStorage() {
	localStorage.setItem(chaveStorage, JSON.stringify(criarPayload()));
}

function carregarDoStorage() {
	const dados = localStorage.getItem(chaveStorage);
	const payload = dados ? JSON.parse(dados) : { alunos: [] };
	alunos = Array.isArray(payload.alunos) ? payload.alunos : [];
	atualizarLista();
}
